...
/**
 * 不支持移动端
 * @param {boolean} [wndActive] - 当前活动窗口是否为激活状态
 */
 ...
 * @param {"bookmark" | "name" | "alias" | "memo" | "av" | "custom"} [options.focusName] - av 为数据库页签，custom 为自定义页签，其余为内置输入框
...
/**
 * @param {number} [timeout] - ms. 0: manual close；-1: always show;
 * @param {string} [type]
 */
