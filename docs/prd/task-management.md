...
#### 需求描述
在编辑项目文档时，用户可直接在笔记中打开事项详情弹框，无需切换到日历或待办视图。

#### 操作方式
| 方式 | 操作 |
|------|------|
| 右键菜单 | 在事项块上右键 → 选择「查看详情」「在日历中查看」 |
| 快捷键 | Ctrl+点击（Windows）或 Cmd+点击（Mac）事项内容 → 打开事项详情 |

#### 验收标准
- [x] 右键事项块时菜单中显示「查看详情」选项
- [x] 点击后打开事项详情弹框
- [x] Ctrl/Cmd+点击事项内容可打开弹框
- [x] 弹框内容与待办 Dock 中一致（项目、任务、时间、状态、链接等）

#### 实现方案
- 监听思源 `open-menu-content`、`click-editorcontent` 事件
- 从 DOM 获取 blockId（`data-node-id`），在 projectStore.items 中匹配 Item
- 复用 `showItemDetailModal(item)` 展示
- 前置条件：文档在配置目录内，且 projectStore 已加载数据
