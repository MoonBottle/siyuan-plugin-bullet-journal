...
### 5.5 待办组件与 Todo Dock 的关系

工作台中的 `todoList` widget 不应再维护一套独立的待办展示模型。它的职责应限定为：

- 作为一个“带预置筛选条件的待办入口”
- 保存该 widget 对应的筛选 preset
- 决定该 widget 在仪表盘卡片中展示哪些待办数据

真正的待办信息展示应尽量复用 `Todo Dock` 已有能力，包括：
...
也就是说，工作台待办组件不是第二套 Todo 视图实现，而是基于同一套 Todo 展示/筛选能力做“预置条件化封装”。
...
其中 `todoList` widget 需要额外强调：

- 仪表盘卡片内允许展示待办摘要，但信息结构应与 Todo Dock 保持同源
- 配置项应围绕“筛选 preset + 展示条数”等轻配置展开
- 不单独发明另一套待办字段、另一套顶部筛选、另一套卡片结构
- 后续如果 Todo Dock 的卡片展示字段发生变化，`todoList` widget 应优先通过共享组件同步，而不是分叉维护
...
对于 Todo 相关能力，需要进一步细化：

1. 工作台中的完整 `todo` 视图应继续复用 `DesktopTodoDock` 的主内容能力
2. 仪表盘中的 `todoList` widget 应尽量复用 Todo 的展示层与筛选模型，而不是维护一套摘要专用实现
3. Todo 顶部筛选条是统一筛选交互入口，`todoList` widget 只提供 preset，不提供另一套独立筛选体系
4. 打开某个 `todoList` widget 对应的待办视图时，顶部筛选的初始值应由该 widget 的 preset 注入
5. 用户在该视图中临时调整顶部筛选时，默认只影响当前查看态，不自动覆盖 widget preset
6. 若后续需要把当前筛选写回 widget preset，应显式提供“保存为当前组件配置”动作，而不是隐式覆盖

这组约束的目标是把 `todoList` widget 的角色固定为“预置筛选入口”，同时保证真正的 Todo 展示仍然只有一套主实现。
