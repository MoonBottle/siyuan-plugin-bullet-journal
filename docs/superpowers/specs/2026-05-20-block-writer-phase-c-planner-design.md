...
3. 当前块 `removeSlashCommand + setContent`
4. 当前块 `setPriority + setContent + addDate`
5. 当前块 habit 定义原地更新与 slash cleanup
...
2. 按 `caretPolicy` 执行 `wbr` 主恢复
3. 仅在 `wbr` 丢失时，由执行层自行走 `offset` 兜底；这不属于 planner 语义
