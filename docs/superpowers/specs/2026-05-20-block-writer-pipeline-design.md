...
2. **`removeSlashCommand` 只做 cleanup，不再承载内容语义**
   ...
   - 所有 slash 命令都必须显式表达为批量 patch：
     - `/rw` -> `removeSlashCommand` + `setContent`
...
3. **恢复目标必须与命令语义一致**
   ...
   - 改写既有 marker：
     ...
     - 光标落在当前块末尾
   - 对已有 marker 集合新增优先级：
     - 例如 `评审视觉稿 📅2026-05-15,2026-05-20 ⏰14:00 /fq`
     - 执行后 `评审视觉稿 📅2026-05-15,2026-05-20 ⏰14:00 🌱|`
...
5. `removeSlashCommand` 不再携带 `suffix`，slash 命令插入内容必须通过显式语义 patch 表达。
6. 结构性 slash 插入或 render 后光标按 `wbr` 语义恢复，而不是简单回到旧 text node。
7. 对已有日期/时间 marker 设置优先级时，结果为 `... ⏰14:00 🌱`，而不是 `... 🌱 📅... ⏰14:00`。
8. `protyleCommitter.ts` 与 `apiCommitter.ts` 对同一 patch 序列产出相同的 marker 顺序。
9. insert 默认走 DOM-first API。
10. DOM 不可用时 markdown fallback 语义不变。
