...
这还不够覆盖“斜杠命令删除后又插入内容”的真实场景。B 阶段设计必须显式对齐思源源码的两级机制：

1. **轻量文本变更走 offset 恢复**
   - 适用场景：
     - 仅删除 slash 命令文本
     - 删除 slash 后追加纯文本 suffix，例如当前项目的 `removeSlashCommand + suffix`
   - 设计要求：
     - 不再保存旧 `Text` 节点引用作为最终方案
     - 应保存相对于当前 editable 容器的逻辑偏移
     - 提交后通过思源同类机制 `focusByOffset()` 恢复
   - 参考：
     - 思源 [`app/src/protyle/util/selection.ts`](C:\dev\projects\open-source\siyuan\app\src\protyle\util\selection.ts) 的 `focusByOffset()`（约 L457）

2. **结构性写回走 `<wbr>` 标记恢复**
   - 适用场景：
     - slash 删除后插入结构化 inline 内容，如 tag / block-ref / image placeholder
     - `renderMarkdownIntoBlockEditable()` 或其他 DOM 重建路径
     - 当前块 slash 后触发 markdown render，导致原 text node 失效
   - 设计要求：
     - 在写入前克隆目标块 DOM，并在预期光标位置插入 `<wbr>`
     - 提交后在新 DOM 中通过 `focusByWbr()` 恢复
     - `<wbr>` 恢复失败时，再退到 offset 或块尾
   - 参考：
...
3. **恢复目标必须与插入语义一致**
   - 纯文本 suffix：光标应折叠在新插入文本之后
   - inline 结构：光标应位于新 inline 节点之后，或思源约定的可继续输入位置
   - 带占位符的结构（如图片、代码块等）：光标应恢复到思源原生 slash 行为对应的位置，而不是简单回到块尾

4. **失败降级策略**
   - `wbr` 恢复失败 -> 尝试 `offset`
   - `offset` 恢复失败 -> 折叠到当前块 editable 末尾
...
- `caretController.ts` 负责 current Protyle 选区快照与恢复，统一封装 `offset` / `wbr` 两级策略；
...
- 纯文本 slash 写回优先走 `focusByOffset` 等价机制
- 结构性 slash 插入优先走 `<wbr>` + `focusByWbr` 等价机制
...
5. `removeSlashCommand + suffix` 后光标折叠在新插入文本之后。
6. 结构性 slash 插入或 render 后光标按 `wbr` 语义恢复，而不是简单回到旧 text node。
7. insert 默认走 DOM-first API。
8. DOM 不可用时 markdown fallback 语义不变。
