...
export const useAIStore = defineStore('ai', () => {
  ...
  interface WeixinConversationStatusResult {
  ...
  async function getOrCreateWeixinConversation(
    ...
    console.log('[AIStore] getOrCreateWeixinConversation:', {
      ilinkUserId,
      userName,
    })
  ...
  async function generateAIReply(
    ...
    console.log('[AIStore] 会话消息:', conversation.messages.map((m) => ({
      role: m.role,
  ...
  async function sendReplyToWeixin(toUserId: string, content: string, contextToken?: string) {
    ...
    console.log('[AIStore] sendReplyToWeixin:', {
      toUserId,
    ...
    try {
      ...
      if (userIds.length === 0 && storageService) {
        ...
        try {
          ...
          console.log('[AIStore] 微信会话:', weixinConvos.length, '条', weixinConvos.map((c: any) => ({
            id: c.id,
            source: c.source,
            weixinUserId: c.weixinUserId,
          })))
