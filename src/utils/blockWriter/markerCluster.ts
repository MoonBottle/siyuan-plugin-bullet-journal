...
const MARKER_PATTERNS: Array<{ kind: MarkerKind; regex: RegExp }> = [
  ...
  { kind: 'reminder', regex: /^⏰(?:\d{2}:\d{2}(?::\d{2})?|提前\d+(?:分钟|小时|天)|结束前\d+(?:分钟|小时|天)|\d+\s*(?:minutes?|hours?|days?|m|h|d)\s*before(?:\s*end)?)$/iu },
  { kind: 'recurring', regex: /^🔁\S+$/u },
  { kind: 'endCondition', regex: /^(?:截止到\d{4}-\d{2}-\d{2}|until\s+\d{4}-\d{2}-\d{2}|剩余\s*\d+\s*次|\d+\s*(?:times?\s*)?remaining)$/iu },
...
function buildParsedLine(segments: MarkerSegment[]): ParsedMarkerLine {
  ...
  const markers = segments
    ...
    .map(segment => ({ kind: segment.kind, raw: segment.raw }));
...
export function parseMarkerLine(line: string): ParsedMarkerLine {
  ...
  for (const token of tokens) {
    ...
    if (kind) {
      segments.push({ type: 'marker', kind, raw: token });
      continue;
    }

    segments.push({ type: 'text', raw: token });
  }
...
export function upsertMarker(parsed: ParsedMarkerLine, kind: MarkerKind, raw?: string): ParsedMarkerLine {
  ...
  if (existingIndex >= 0) {
    nextSegments[existingIndex] = { type: 'marker', kind, raw };
  ...
  if (lastMarkerIndex >= 0) {
    nextSegments.splice(lastMarkerIndex + 1, 0, { type: 'marker', kind, raw });
  } else {
    nextSegments.push({ type: 'marker', kind, raw });
  }
