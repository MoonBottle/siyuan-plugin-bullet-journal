...
/** 专注计划变更：设置或清除专注计划 */
export interface FocusPlanPatch {
  type: 'setFocusPlan'
...
/** 习惯定义变更：设置习惯的属性 */
export interface HabitDefinitionPatch {
  type: 'setHabitDefinition'
  habit: Partial<Habit>
}

/** 习惯打卡记录变更：写入一条习惯打卡数据 */
export interface HabitRecordPatch {
  type: 'setHabitRecord'
  record: {
    content: string
...
/** 习惯归档变更：设置习惯的归档时间 */
export interface HabitArchivePatch {
  type: 'setHabitArchive'
  archivedAt?: string
}

/** Markdown 整块替换：用新 markdown 内容替换整个块，可选保留 IAL 属性 */
export interface ReplaceMarkdownPatch {
