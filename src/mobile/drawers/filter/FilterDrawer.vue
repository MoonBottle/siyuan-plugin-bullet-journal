<template>
  <Teleport to="body">
          ...
          >
            ...
            <div class="sheet-header">
              <h4 class="sheet-title">
                {{ t('mobile.filter.projectGroup') || '项目分组' }}
              </h4>
            </div>
            <div
              class="sheet-content"
    ...
    <Transition name="fade">
      <div
        v-if="showDateFilterSheet"
        class="sheet-overlay b3-dialog"
        @click="closeDateFilterSheet"
      >
        <Transition name="slide-up">
          <div
            v-if="showDateFilterSheet"
          ...
          >
            ...
            <div class="sheet-header">
              <h4 class="sheet-title">
                {{ t('mobile.filter.dateFilter') || '日期筛选' }}
              </h4>
            </div>
            <div
              class="sheet-content"
            ...
            >
              ...
              >
                ...
                <svg
                  v-if="localDateFilter === opt.value"
                  class="check-icon"
                ><use xlink:href="#iconCheck"></use></svg>
              </button>
            </div>
...
const dateOptions = [
  ...
  {
    value: 'thisWeek',
    label: t('todo').dateFilter?.thisWeek || '本周',
  },
  {
    value: 'thisMonth',
    label: t('todo').dateFilter?.thisMonth || '本月',
  },
  {
    value: 'recent7',
    label: t('todo').dateFilter?.recent7 || '近7天',
  },
  {
    value: 'all',
    label: t('todo').dateFilter?.all || '全部',
  },
  {
    value: 'custom',
    label: t('todo').dateFilter?.custom || '自定义',
  },
]
...
const priorityOptions = [
  {
    ...
    label: t('todo').priority?.high || '高优先级',
    emoji: '🔥',
  },
  {
    value: 'medium' as PriorityLevel,
    label: t('todo').priority?.medium || '中优先级',
    emoji: '🌱',
  },
  {
    value: 'low' as PriorityLevel,
    label: t('todo').priority?.low || '低优先级',
    emoji: '🍃',
  },
]
