...
async function request(url: string, data: any) {
  ...
  if (response.code !== 0) {
    console.error(`[Task Assistant] API request failed: ${url}`, {
      code: response.code,
      msg: response.msg,
      data: response.data,
    })
 ...
 * 与 getBlockKramdown 相同，但语义上更清晰用于获取文档
 */
export async function getDocKramdown(
  docId: DocumentId,
): Promise<IResGetBlockKramdown> {
...
export async function openFileById(id: string, action?: string) {
  ...
  const url = "/api/filetree/openDoc"
  return request(url, data)
}
