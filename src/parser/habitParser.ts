...
export function parseHabitLine(line: string): Partial<Habit> | null {
  ...
  const countMatch = searchArea.match(/(\d+)([a-z\u4E00-\u9FFF]{1,4})$/i)
...
export function parseCheckInRecordLine(line: string, habitId: string): Partial<CheckInRecord> | null {
  ...
  const hasCompletedMarker = /(?:^|\s)✅$/.test(normalizedLine)
  ...
  // 解析计数格式 N/M单位 (如 3/8杯)
  const countMatch = normalizedLine.match(/(\d+)\/(\d+)([a-z\u4E00-\u9FFF]+)/i)
...
export function parseHabitRecordLine(line: string, habitId: string): Partial<CheckInRecord> | null {
  ...
  const hasHabitRecordMarkers = /\d+\/\d+[a-z\u4E00-\u9FFF]+/i.test(normalizedLine) || /(?:^|\s)❌$/.test(normalizedLine) || /(?:^|\s)✅$/.test(normalizedLine)
