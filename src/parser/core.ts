...
export function parseKramdown(
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (taskListMatch && taskListCheckboxMatch) {
      ...
      while (listItemBlockIdStack.length > 0 && listItemBlockIdStack.at(-1).level >= indentLevel) {
        listItemBlockIdStack.pop()
      }
      // 将当前任务列表项压入栈（使用从行内提取的列表项 ID，而不是 block.blockId）
      listItemBlockIdStack.push({
        id: listItemBlockId,
        level: indentLevel,
      })
    }
  ...
  if (!project.name) {
    if (docPath) {
      ...
      const fallbackName = pathParts.at(-1) || pathParts[pathParts.length - 2] || docPath
