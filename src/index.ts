...
export default class TaskAssistantPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.addIcons(ICON_BRUSH_CLEANING)
  ...
  private updateStatusBarTimerDisplay(
    ...
    if (iconEl) {
      ...
      if (isBreak) {
        // 咖啡图标
        iconEl.innerHTML = `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><use xlink:href="#iconTaCoffee" /></svg>`
      } else {
        // 番茄图标
        iconEl.innerHTML = `<svg viewBox="0 0 1024 1024" width="14" height="14" fill="currentColor"><use xlink:href="#iconTaTomato" /></svg>`
      }
    }
