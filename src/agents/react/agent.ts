...
import { executeToolCalls } from '@/services/aiToolsExecutor'
...
export class ReActAgent extends EventEmitter<AgentEvents> {
  ...
  async run(userInput: string, conversation: ConversationData): Promise<void> {
    ...
    try {
      ...
      while (this.state.currentIteration < this.context.maxIterations) {
        ...
        // Step 3: Action - 执行工具
        console.log(`${logPrefix} 执行 ${thought.toolCalls.length} 个工具调用:`, thought.toolCalls.map((tc) => ({
          id: tc.id,
  ...
  private async think(): Promise<AIResponse & { messageId: string }> {
    ...
    console.log(`${logPrefix} 发送给API的消息:`, messages.map((m, i) => ({
      ...
      contentLength: m.content?.length || 0,
      hasToolCalls: !!m.toolCalls?.length,
      toolCallsInfo: m.toolCalls?.map((tc) => ({
        id: tc.id,
        name: tc.function.name,
      })),
      toolCallId: m.toolCallId,
  ...
  private updateAssistantToolCalls(id: string, toolCalls: ToolCall[]): void {
    ...
    if (message && message.role === 'assistant') {
      console.log(`${logPrefix} 更新消息 ${id.substring(0, 20)} 的 toolCalls:`, toolCalls.map((tc) => ({
        id: tc.id,
        name: tc.function.name,
      })))
  ...
  private validateToolCallMessages(messages: ChatMessage[], logPrefix: string): void {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (m.role === 'assistant' && m.toolCalls && m.toolCalls.length > 0) {
        for (const tc of m.toolCalls) {
          ...
          console.log(`${logPrefix} 检测到tool_call:`, {
            id: tc.id,
            name: tc.function.name,
            atIndex: i,
          })
      ...
      if (m.role === 'tool' && m.toolCallId) {
        if (pendingToolCallIds.has(m.toolCallId)) {
          ...
          console.log(`${logPrefix} 检测到tool结果:`, {
            toolCallId: m.toolCallId,
            atIndex: i,
            resolved: true,
          })
        } else {
          console.warn(`${logPrefix} 警告: tool消息没有对应的tool_call:`, {
            toolCallId: m.toolCallId,
            atIndex: i,
          })
        }
    ...
    if (pendingToolCallIds.size > 0) {
      console.error(`${logPrefix} 错误: 有 ${pendingToolCallIds.size} 个 tool_call 没有对应的 tool 消息:`, Array.from(pendingToolCallIds))
    }
  }
