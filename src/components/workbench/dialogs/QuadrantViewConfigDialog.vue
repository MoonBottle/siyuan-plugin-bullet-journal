<template>
  <WorkbenchConfigDialogLayout>
    <div class="quadrant-view-config-dialog__body">
      <div class="quadrant-view-config-dialog__field">
        <label class="quadrant-view-config-dialog__label">
          {{ t('settings').projectGroups.title }}
        </label>
        <SySelect
          v-model="selectedGroup"
          data-testid="quadrant-view-config-group-select"
    ...
    <template #footer>
      <button
        ...
        data-testid="quadrant-view-config-cancel"
        type="button"
        @click="onCancel"
      >
        {{ t('common').cancel }}
      ...
      <button
        ...
        data-testid="quadrant-view-config-confirm"
        type="button"
        @click="handleConfirm"
      >
        {{ t('common').confirm }}
...
const props = defineProps<{
  ...
  onConfirm: (config: WorkbenchQuadrantViewConfig) => void;
  onCancel: () => void;
}>();
...
.quadrant-view-config-dialog__body {
  display: flex;
...
.quadrant-view-config-dialog__field {
  display: flex;
  flex-direction: column;
  gap: 8px;
}

.quadrant-view-config-dialog__label {
  font-size: 13px;
  font-weight: 500;
