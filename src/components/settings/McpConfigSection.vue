<template>
  ...
  <template v-if="!isMobile">
    ...
    >
      ...
      <SySettingsActionButton
        icon="iconCopy"
        :text="(t('settings') as any).mcp?.httpCopyButton ?? '复制 HTTP MCP 配置'"
      ...
      <div class="ios-group-header">
        ...
        <div class="header-info">
          <div class="header-title">
            {{ (t('settings') as any).mcp?.title ?? 'MCP 配置' }}
          </div>
          <div class="header-desc">
            {{ (t('settings') as any).mcp?.description ?? '将任务数据暴露给 Cursor、Claude 等 AI 助手' }}
          </div>
        </div>
      </div>
      ...
      <div class="ios-group">
        <div class="ios-card">
          <div class="ios-cell ios-cell-info">
            ...
            <div class="info-text">
              {{ (t('settings') as any).mcp?.mobileHint ?? 'MCP 配置需要在桌面端复制后使用' }}
            </div>
          </div>
        </div>
        <button
