...
describe('TaskAssistantPlugin mobile openSetting behavior', () => {
  it('opens the mobile main shell instead of the settings dialog on mobile', () => {
    ...
    expect(indexSource).toMatch(
      /openSetting\(\): void \{\s*void this\.loadSettings\(\)\.then\(\(\) => \{\s*if \(this\.isMobile\) \{\s*this\.openMobilePluginDock\(DOCK_TYPES\.TODO,\s*"todo"\);\s*return;\s*\}\s*showSettingsDialog\(this\);\s*\}\);\s*\}/s,
    );
  });
});
