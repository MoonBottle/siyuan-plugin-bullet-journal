...
import { executeTool } from '@/services/aiToolsExecutor'

const {
  mockWriteBlock,
  mockInsertBlockAfterWithResult,
} = vi.hoisted(() => ({
...
describe('executeTool', () => {
  ...
  it('routes update_item_status through writeBlock for completed state', async () => {
    ...
    const raw = await executeTool(
      createToolCall('update_item_status', {
        itemId: 'item-1',
        status: 'completed',
      }),
      createContext(item),
