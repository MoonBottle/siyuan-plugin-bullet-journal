...
describe('clawBotProxyServer', () => {
  ...
  function createServer(opts: { preferredPort?: number; maxPort?: number } = {}) {
    ...
    return createClawBotProxyServer({ ...opts, maxPort, fetchFn: fetchMock });
  }
  ...
  it('throws when all ports in range are occupied', async () => {
    ...
    try {
      for (let i = 0; i < 5; i++) {
        servers.push(await createServer({ preferredPort: 18980, maxPort: 18984 }));
      }
      await expect(
        createServer({ preferredPort: 18980, maxPort: 18984 }),
