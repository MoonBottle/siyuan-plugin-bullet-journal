import type { FilterItemOutput } from '@/mcp/filterItems'
...
import { executeFilterItems } from '@/mcp/filterItems'
...
describe.skip('filter_items 集成测试 - 多日期事项', () => {
  ...
  describe('链接测试（缩进格式）', () => {
    it('链接缩进A: date=2026-03-10', async () => {
      ...
      expect(items[0].links![0]).toMatchObject({
        name: '链接A1',
    ...
    it('链接缩进C: date=2026-03-15', async () => {
      ...
      expect(items[0].links![1]).toMatchObject({
        name: '链接C2',
