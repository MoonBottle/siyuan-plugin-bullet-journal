...
describe('parseBlockRefs 块引用解析', () => {
  it('单引号锚文本', () => {
    const {
      stripped,
      links,
    } = parseBlockRefs("首页((20260310210016-gkixdit '测试'))改版")
  ...
  it('双引号锚文本', () => {
    const {
      stripped,
      links,
    } = parseBlockRefs('((20200813131152-0wk5akh "在内容块中遨游"))')
    expect(stripped).toBe('在内容块中遨游')
    expect(links).toHaveLength(1)
    expect(links[0]).toEqual({
      name: '在内容块中遨游',
      url: 'siyuan://blocks/20200813131152-0wk5akh',
      type: 'block-ref',
    })
  })
  ...
  it('无锚文本替换为空', () => {
    const {
      stripped,
      links,
    } = parseBlockRefs('((20260310210016-gkixdit))纯文本')
...
describe('parseItemLine - 事项链接', () => {
  ...
  it('事项带多个链接', () => {
    const links = [
      ...
      {
        name: '设计稿',
        url: 'siyuan://blocks/20260220112000',
      },
    ]
    const items = LineParser.parseItemLine('工作事项 @2024-01-01', 1, links)
    expect(items).toHaveLength(1)
  ...
  it('事项内容含块引用：strip 并提取到 links', () => {
    ...
    expect(items[0].links).toHaveLength(1)
    expect(items[0].links![0]).toEqual({
      name: '别名',
      url: 'siyuan://blocks/20260310210016-gkixdit',
      type: 'block-ref',
    })
  ...
  it('事项块引用与下方链接合并', () => {
    ...
    expect(items[0].links![1]).toEqual({
      name: '块引用',
      url: 'siyuan://blocks/20260310210016-gkixdit',
      type: 'block-ref',
    })
...
describe('parsePomodoroLine 带块属性解析', () => {
  it('解析带 running 状态的番茄钟', () => {
    const attrs = {
      ...
      'custom-pomodoro-item-content': '测试事项',
    }
...
describe('parseItemLine - 重复规则解析', () => {
  ...
  it('🔁每月15日 应被正确识别指定日期', () => {
    ...
    expect(items[0].repeatRule).toEqual({
      type: 'monthly',
      dayOfMonth: 15,
    })
  })
  ...
  it('带结束日期的重复规则', () => {
    ...
    expect(items[0].repeatRule).toEqual({ type: 'daily' })
    expect(items[0].endCondition).toEqual({
      type: 'date',
      endDate: '2026-04-30',
    })
  })
