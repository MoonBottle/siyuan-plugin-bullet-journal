...
describe('parseItemLine - 斜杠命令移除', () => {
  it('移除 /todo 斜杠命令', () => {
    const items = LineParser.parseItemLine('整理资料 @2024-01-01 /todo', 1);
    expect(items).toHaveLength(1);
    expect(items[0].content).toBe('整理资料');
    expect(items[0].date).toBe('2024-01-01');
  });
  ...
  it('移除 /zz 斜杠命令', () => {
    const items = LineParser.parseItemLine('专注工作 @2024-01-01 /zz', 1);
    expect(items).toHaveLength(1);
    expect(items[0].content).toBe('专注工作');
  });

  it('移除 /sx 斜杠命令', () => {
    const items = LineParser.parseItemLine('标记事项 @2024-01-01 /sx', 1);
    expect(items).toHaveLength(1);
    expect(items[0].content).toBe('标记事项');
  });

  it('移除 /db 斜杠命令', () => {
    const items = LineParser.parseItemLine('查看待办 @2024-01-01 /db', 1);
    expect(items).toHaveLength(1);
    expect(items[0].content).toBe('查看待办');
  });

  it('移除 /calendar 斜杠命令', () => {
    const items = LineParser.parseItemLine('打开日历 @2024-01-01 /calendar', 1);
    expect(items).toHaveLength(1);
    expect(items[0].content).toBe('打开日历');
  });

  it('多日期事项移除斜杠命令', () => {
    const items = LineParser.parseItemLine('整理资料 @2024-01-01, 2024-01-03 /todo', 1);
    expect(items).toHaveLength(2);
    expect(items[0].content).toBe('整理资料');
    expect(items[1].content).toBe('整理资料');
  });
  ...
  it('带状态标签和斜杠命令', () => {
    const items = LineParser.parseItemLine('整理资料 @2024-01-01 #done /todo', 1);
    expect(items).toHaveLength(1);
    expect(items[0].content).toBe('整理资料');
    expect(items[0].status).toBe('completed');
  });

  it('内容中包含斜杠但不是命令（不在行尾）', () => {
    const items = LineParser.parseItemLine('整理资料 / 查看 @2024-01-01', 1);
    expect(items).toHaveLength(1);
    expect(items[0].content).toBe('整理资料 / 查看');
  });
  ...
  it('移除中文斜杠命令 /甘特图', () => {
    const items = LineParser.parseItemLine('查看甘特图 @2024-01-01 /甘特图', 1);
    expect(items).toHaveLength(1);
    expect(items[0].content).toBe('查看甘特图');
  });

  it('移除中文斜杠命令 /日历', () => {
    const items = LineParser.parseItemLine('打开日历 @2024-01-01 /日历', 1);
    expect(items).toHaveLength(1);
    expect(items[0].content).toBe('打开日历');
  });

  it('移除中文斜杠命令 /事项', () => {
    const items = LineParser.parseItemLine('标记事项 @2024-01-01 /事项', 1);
    expect(items).toHaveLength(1);
    expect(items[0].content).toBe('标记事项');
  });

  it('移除中文斜杠命令 /专注', () => {
    const items = LineParser.parseItemLine('开始专注 @2024-01-01 /专注', 1);
    expect(items).toHaveLength(1);
    expect(items[0].content).toBe('开始专注');
  });

  it('移除中文斜杠命令 /待办', () => {
    const items = LineParser.parseItemLine('查看待办 @2024-01-01 /待办', 1);
    expect(items).toHaveLength(1);
    expect(items[0].content).toBe('查看待办');
  });

  it('移除中英文混合斜杠命令 /calendar', () => {
    const items = LineParser.parseItemLine('打开日历 @2024-01-01 /calendar', 1);
    expect(items).toHaveLength(1);
    expect(items[0].content).toBe('打开日历');
  });
